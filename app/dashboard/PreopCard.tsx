...
export default function Dashboard() {
  ...
  async function loadPatient(rawFolder: string) {
    try {
      ...
      setDemographic(
        prepareDemographicData(caseInfo, patientAttr, preopRow, id)
      );
  ...
  const hemoSeries = buildSeries(
    vitals.hemodynamics,
    (k) => PANEL_COLORS.hemodynamics[k as keyof typeof PANEL_COLORS.hemodynamics] ?? "#999"
  );
        ...
        <div className="flex min-h-0 flex-1 flex-col space-y-6">
          <div className="flex items-center justify-between">
            ...
            <div className="flex items-center gap-3">
              ...
              {submitError && (
                <span className="text-sm font-semibold text-red-600">{submitError}</span>
              )}
